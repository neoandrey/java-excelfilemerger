...
public class ExcelFileMerger{
		...
		public ExcelFileMerger(String source, String target){
			...
			if(sheets[k].getRow(i) != null){
				...
				XSSFRow row = sheets[k].getRow(i);
					for (int j=0; j<colNum; j++){
					    ...
					    if(cell != null && cell.getCellType()!=XSSFCell.CELL_TYPE_BLANK) value = xssfCellToString(cell);
					    else value = null;
						excelData[0][j] = value;
			...
			excelData = new String[1][colNum];
				HSSFRow row = sheets[k].getRow(i);
					for (int j=0; j<colNum; j++){
					   ...
					   if(cell != null && cell.getCellType()!=HSSFCell.CELL_TYPE_BLANK) value = hssfCellToString(cell);
					   else value = null;
						excelData[0][j] = value;
...
return result.toString();
	...
	if(processOutputFile.exists()){
		...
		public static void showUsage(){
			...
			System.out.println("Usage:  ExcelFileMerger -s 'C:\\temp' -t 'C:\\users\\public'");
	...
	public static void main(String [] args){
		...
		if(args.length==0){
				...
				try{
				   ...
				   new ExcelFileMerger( source,  target);
				   System.out.println("\n\nProcess Complete.");
				   System.out.println("\nExiting...");
